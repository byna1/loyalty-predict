...
def date_rage(start,stop,monthly=False): 
    dates = []
    while start <= stop: 
        dates.append(start)
...
def exec_query(db_origin,table,db_target,start,stop,monthly):
    ...
    dates = date_rage(start, stop,monthly)
...
def main():
    ...
    parser.add_argument("--db_origin",choices=['analytics','loyalty-system','ed-platform'],default='loyalty-system')
    ...
    parser.add_argument("--start",type=str,default=now,help="Table that will be processed with the same name as the file")
    parser.add_argument("--stop",type=str,default=now)
    parser.add_argument("--monthly",action='store_true')

    args = parser.parse_args()
    exec_query(args.db_origin,args.table,args.db_target,args.start,args.stop,args.monthly)
